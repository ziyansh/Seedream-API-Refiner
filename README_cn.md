...
- Python 3.10+
- 互联网连接（用于调用 Seedream API）
- 足够的磁盘空间（用于保存优化后的图像）

## 示例工作流

1. **基础优化**：启用扩展 → 输入提示词 → 生成图像 → 自动优化

2. **风格参考**：启用扩展 → 上传风格参考图像 → 生成图像 → 基于参考风格优化

3. **结构控制**：启用扩展 → 配置 Canny 控制单元 → 上传边缘参考 → 生成图像 → 保持结构优化

4. **姿态保持**：启用扩展 → 配置 OpenPose 控制单元 → 上传姿态参考 → 生成图像 → 保持姿态优化
